...
class Atom {
    ...
    get electronsArray() {
        let arr=[]; // le code html qui sera renvoyé
        ...
        const cDouble = [
            {x: 45, y:15, i:85, j:15},
            {x: 115, y:45, i:115, j:85},
            {x: 45, y:115, i:85, j:115},
            {x: 15, y:45, i:15, j:85},
        ]
        ...
        if(n<=4) {
            for(let i=1; i<n+1; i++) {
                arr.push({n:i, type:'single', build: function(ctx){
                    ctx.beginPath();
            ...
            let j=1;
            for(j; j<n-3; j++) {
                arr.push({n:j, type: 'double', build: function(ctx){
                    ctx.beginPath();
                    ctx.moveTo(cDouble[this.n-1].x, cDouble[this.n-1].y);
